...
@section('content')
<h1 class="h3 mb-4 text-gray-800">Edit Data Kartu</h1>
    ...
    <div class="card-body">
        <form action="{{ route('cards.update', $card->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('cardno') is-invalid @enderror" id="cardno" name="cardno" value="{{ old('cardno', $card->cardno) }}">
            ...
            <div class="form-group">
                ...
                <div>
                    <div class="custom-control custom-radio custom-control-inline">
                        <input type="radio" id="type_employee" name="type" class="custom-control-input" value="employee" {{ old('type', $card->type) == 'employee' ? 'checked' : '' }}>
                        <label class="custom-control-label" for="type_employee">Karyawan</label>
                    </div>
                    <div class="custom-control custom-radio custom-control-inline">
                        <input type="radio" id="type_guest" name="type" class="custom-control-input" value="guest" {{ old('type', $card->type) == 'guest' ? 'checked' : '' }}>
            ...
            <div class="form-group" id="employee-field">
                ...
                <select class="form-control @error('cardable_id') is-invalid @enderror" id="cardable_id" name="cardable_id">
                    ...
                    @foreach ($employees as $employee)
                        <option value="{{ $employee->id }}" {{ old('cardable_id', $card->cardable_id) == $employee->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control @error('is_active') is-invalid @enderror" id="is_active" name="is_active">
                    <option value="1" {{ old('is_active', $card->is_active) == '1' ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ old('is_active', $card->is_active) == '0' ? 'selected' : '' }}>Non-Aktif</option>
                </select>
